...
function scanDirectory(dir, extensions) {
  ...
  for (const item of items) {
    ...
    if (item.isDirectory()) {
      files = files.concat(scanDirectory(fullPath, extensions)); // Recursively scan subdirectories
    } else if (extensions.includes(path.extname(item.name).toLowerCase())) {
      files.push(fullPath); // Add file if it matches the extensions
    }
  }
...
function buildGallery(platform, gamesDir, emulator, emulatorArgs, userDataPath) {
  ...
  const gameFiles = scanDirectory(gamesDir, extensions);
